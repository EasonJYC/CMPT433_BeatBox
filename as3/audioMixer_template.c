...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	for(int i = 0; i < MAX_SOUND_BITES; i++)
	{
		...
		else
		{
			...
			int total = soundBites[i].pSound -> numSamples;
			for(int j = 0; j < size; j++)
			{
				if(offset < total)
				{
					short value = soundBites[i].pSound -> pData[offset];
					if(value + playbackBuffer[j] - SHRT_MAX > 0){
						playbackBuffer[j] = SHRT_MAX;
						offset++;
					}
					else if(value + playbackBuffer[j] - SHRT_MIN < 0){
			...
			//check if entire sample is played
			if(offset == total)
			{
				soundBites[i].pSound = NULL;
				soundBites[i].location = 0;
